...
import com.excilys.computerDatabase.core.exception.dao.ComputerNotFoundException;
...
public class ComputerDAO {
	...
	public long count(String search) {
		try {
			...
			criteriaQuery.select(critriaBuilder.count(rootComputer)).where(critriaBuilder
					.or(withComputerNameContainSearch, withCompanyNameContainSearch, withCompanyNameContainSearch));
	...
	public List<Computer> find(Page page) {
		...
		try {
			...
			Root<ComputerEntity> rootComputer = criteriaQuery.from(ComputerEntity.class);
			Join<ComputerEntity, CompanyEntity> rootCompany = rootComputer.join("company", JoinType.LEFT);
			Predicate withComputerNameContainSearch = critriaBuilder.like(rootComputer.get("name"),
					"%" + page.getSearch() + "%");
	...
	public void update(Computer computer) {
		try {
			...
			CriteriaUpdate<ComputerEntityAdd> criteriaUpdate = critriaBuilder
					.createCriteriaUpdate(ComputerEntityAdd.class);
